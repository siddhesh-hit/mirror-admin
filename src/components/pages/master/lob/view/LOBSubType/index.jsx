...
const LOBSubType = ({ data, paths, handleDelete, pageOptions, setPageOptions }) => {
        ...
        <React.Fragment>
            <TotalEntries
                returnCount={(data) =>
                    setPageOptions((prev) => ({
                        ...prev,
                        lobSubType: { ...prev.lobSubType, page: data },
                    }))
                }
                returnSearch={(data) =>
                    setPageOptions((prev) => ({
                        ...prev,
                        lobSubType: { ...prev.lobSubType, topic_name: data }
                    }))
                }
                                ...
                                {data?.data && data?.data?.length > 0 ? (
                                    data?.data?.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <h4>{item.lobType?.name ? item.lobType?.name : "-"}</h4>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <Link onClick={() => handleDelete(item._id, "lobSubType")}>
                                                    <OverlayTrigger
                                                        delay={{ hide: 450, show: 300 }}
                                                        overlay={(props) => (
